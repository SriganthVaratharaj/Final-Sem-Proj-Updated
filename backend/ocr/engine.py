...
MODEL_SUPPORTED_CODES = {'latin', 'devanagari', 'ta', 'te', 'ka', 'arabic'}
...
MODEL_LANGUAGE_CODES = {
    ...
    # Indic scripts not directly supported by this Paddle build.
    'bengali': None,
    'gurmukhi': None,
    'gujarati': None,
    'oriya': None,
...
def _init_model(lang):
    ...
    try:
        return PaddleOCR(lang=lang, use_angle_cls=True, use_space_char=True, show_log=False, enable_mkldnn=True)
...
def run_ocr(image, lang=None, image_bytes: bytes = None):
    """
    ...
    is_gibberish = avg_conf < 0.45 or (len(texts) > 0 and len("".join(texts).strip()) < 5)
    
    if is_gibberish and image_bytes:
        logger.info("[ocr] Results look like gibberish (conf=%.2f). Switching to GGUF fallback...", avg_conf)
        return run_ocr_gguf(image_bytes)
